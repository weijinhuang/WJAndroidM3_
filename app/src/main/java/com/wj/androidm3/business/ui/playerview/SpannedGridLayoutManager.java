 ...
 */
public class SpannedGridLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler {
    ...
    private void calculateWindowSize() {
        ...
        //Longse start
        //切换到全屏时，高度不再按比例计算
        final DisplayMetrics displayMetrics = WJApplication.getINSTANCE().getResources().getDisplayMetrics();
        final int screenHeight = Math.min(displayMetrics.widthPixels,displayMetrics.heightPixels);
        final int cellPortraitHeight = (int) Math.floor(screenHeight * (1f / cellAspectRatio));
        if (cellHeight != cellPortraitHeight){
            //当前是全屏
            final int itemMin = screenHeight / 4;
            if (cellHeight > itemMin){
                cellHeight = itemMin;
            }
        }
        //Longse end
