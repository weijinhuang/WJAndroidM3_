...
class HardwarePipeline(
    ...
    private val renderHandler =
        RenderHandler(renderThread.looper, width, height, fps, filterOn, transfer, dynamicRange, characteristices, encoder, viewFinder)
    ...
    override fun destroyWindowSurface() {
        renderHandler.sendMessage(renderHandler.obtainMessage(RenderHandler.MSG_DESTROY_WINDOW_SURFACE))
    ...
    override fun createResources(surface: Surface) {
        renderHandler.sendMessage(renderHandler.obtainMessage(RenderHandler.MSG_CREATE_RESOURCES, 0, 0, surface))
    }
    ...
    override fun actionDown(encoderSurface: Surface) {
        renderHandler.sendMessage(renderHandler.obtainMessage(RenderHandler.MSG_ACTION_DOWN, 0, 0, encoderSurface))
    }

    override fun clearFrameListener() {
        renderHandler.sendMessage(renderHandler.obtainMessage(RenderHandler.MSG_CLEAR_FRAME_LISTENER))
        renderHandler.waitClearFrameListener()
    }


    override fun cleanup() {
        renderHandler.sendMessage(renderHandler.obtainMessage(RenderHandler.MSG_CLEANUP))
    ...
    private class RenderHandler(
        ...
        fun createRecordRequest(session: CameraCaptureSession, previewStabilization: Boolean): CaptureRequest {
        ...
        private fun copyRenderToEncode() {
            ...
            //swap width and height if the camera is rotated on it side
        ...
        private fun createSyncFence(): SyncFence? {
            ...
            checkEglError("eglCreateSync")
            glFlush()
            checkGlError("glFlush")
            return eglSync?.let {
                val fence = EGLExt.eglDupNativeFenceFDANDROID(eglDisplay, it)
                checkEglError("eglDupNativeFenceFDANDROID")
                fence
            }
        }

        private fun actionDown(encoderSurface: Surface) {
            val surfaceAtrribs = intArrayOf(EGL14.EGL_NONE)
            eglEncoderSurface = EGL14.eglCreateWindowSurface(eglDisplay, eglConfig, encoderSurface, surfaceAtrribs, 0)
            if (eglEncoderSurface == EGL_NO_SURFACE) {
                val error = EGL14.eglGetError()
                throw RuntimeException("Failed to create EGL encoder surface: eglGetError = 0x ${Integer.toHexString(error)}")
            }
        }

        private fun clearFrameListener() {
            cameraTexture.setOnFrameAvailableListener(null)
            cvClearFramedListener.open()
        }

        public fun waitClearFrameListener() {
            cvClearFramedListener.block()
        }
