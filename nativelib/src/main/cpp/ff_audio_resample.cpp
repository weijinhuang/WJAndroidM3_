...
/**
 * 初始化avframe，包括音视频格式、采样数、声道布局、采样率
 * */
...
static int initConvertedSamples(AudioResample **pResample, uint8_t ***converted_input_samples, int frame_size) {
    ...
    *converted_input_samples = (uint8_t **) calloc(ar->outCodecCtx->channels, sizeof(**converted_input_samples));
    if ((ret = av_samples_alloc(*converted_input_samples,
        ...
        LOGE(RESAMPLE_TAG, "av_sample_alloc error:%s", av_err2str(ret));
...
int FFAudioResample::openInput(const char *inputPath) {
    ...
    if (ret < 0) {
        LOGE(RESAMPLE_TAG, "avformat_open_input error : %s", av_err2str(ret));
    }
    ...
    if (!codec) {
        LOGE(RESAMPLE_TAG, "can not find decoder:%s", avcodec_get_name(audioStream->codecpar->codec_id));
        return -1;
    }
    ...
    if (ret < 0) {
        LOGE(RESAMPLE_TAG, "avcodec_open2 error:%s", av_err2str(ret));
    }
    resample->inFrame = av_frame_alloc();
    return ret;
}

int FFAudioResample::openOutput(const char *outputPath, int sample_rate) {
    ...
    if (!codec) {
        LOGE(RESAMPLE_TAG, "avcodec_find_encoder error:%s ", resample->inCodecCtx->codec->name);
        return -1;
    }
...
int FFAudioResample::encodeAudioFrame(AVFrame *frame, int *data_present) {
    ...
    if (*data_present && (ret = av_write_frame(resample->outFormatCtx, &resample->outPacket)) < 0) {
        LOGE(RESAMPLE_TAG, "av_write_frame error:%s", av_err2str(ret));
    }
...
int FFAudioResample::decodeAndCovert(int *finished) {
    ...
    if (data_present) {
        int dst_nb_sample = av_rescale_rnd(resample->inFrame->nb_samples, resample->outCodecCtx->sample_rate, resample->inCodecCtx->sample_rate,
                                           AV_ROUND_UP);
        if (initConvertedSamples(&resample, &dst_samples, dst_nb_sample)) {
            goto end;
        }
        ret = swr_convert(resample->resampleCtx,
                          dst_samples,
                          dst_nb_sample,
                          (const uint8_t **) resample->inFrame->extended_data,
                          resample->inFrame->nb_samples);
        if (ret < 0) {
            LOGE(RESAMPLE_TAG, "resample error=%s", av_err2str(ret));
            goto end;
        }
        av_audio_fifo_write(resample->fifo, (void **) dst_samples, dst_nb_sample);
    }
    ret = 0;
    end:
    if (dst_samples) {
...
int FFAudioResample::resampling(const char *inputPath, const char *outputPath, int sampleRate) {
    ...
    resample->fifo = av_audio_fifo_alloc(resample->outCodecCtx->sample_fmt, resample->outCodecCtx->channels, 10240);
    //写入媒体头
    ret = avformat_write_header(resample->outFormatCtx, nullptr);
    if (ret < 0) {
        LOGE(RESAMPLE_TAG, "avformat_write_header error:%s", av_err2str(ret));
        goto end;
    }
    ...
    while (true) {
        ...
        while (av_audio_fifo_size(resample->fifo) < frame_size) {
            if (decodeAndCovert(&finished) < 0) {
                break;
            }
        }
        //编码
        while (av_audio_fifo_size(resample->fifo) >= frame_size || (finished && av_audio_fifo_size(resample->fifo) > 0)) {
            if (encodeAndWrite() < 0) {
                goto end;
            }
        }
