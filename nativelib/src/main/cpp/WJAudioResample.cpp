...
int WJAudioReSample::startResample(const char *inFilePath, int src_sample_rate, int src_channel_count, int src_sample_format, const char *outFilePath,
    ...
    AVSampleFormat out_sample_fmt = dst_sample_format == 1 ? AV_SAMPLE_FMT_FLT : AV_SAMPLE_FMT_S16;
    int out_sample_rate = dst_sample_rate;
    LOGI(LOG_TAG, "输出参数: out_ch_layout:%d, out_sample_fmt:%d, AV_SAMPLE_FMT_FLT %d", AV_CH_LAYOUT_MONO, out_sample_fmt, out_sample_rate);

    int64_t in_ch_layout = src_channel_count == 1 ? AV_CH_LAYOUT_MONO : AV_CH_LAYOUT_STEREO;;
    AVSampleFormat in_sample_fmt = src_sample_format == 1 ? AV_SAMPLE_FMT_FLT : AV_SAMPLE_FMT_S16;
