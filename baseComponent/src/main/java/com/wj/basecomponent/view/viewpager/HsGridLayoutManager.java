 ...
 */
public class HsGridLayoutManager extends HsLinearLayoutManager {

    private int spanCount = 2;

    private static final boolean DEBUG = true;
    private static final String TAG = "HsGridLayoutManager";
     ...
     */
    public HsGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                               int defStyleRes) {
    ...
    /**
     * Creates a vertical HsGridLayoutManager
     *
     * @param context   Current context, will be used to access resources.
     * @param spanCount The number of columns in the grid
     */
    public HsGridLayoutManager(Context context, int spanCount) {
        super(context);
    ...
    /**
     * stackFromEnd is not supported by HsGridLayoutManager. Consider using
    ...
    public void setStackFromEnd(boolean stackFromEnd) {
        if (stackFromEnd) {
            throw new UnsupportedOperationException(
                    "HsGridLayoutManager does not support stack from end."
...
//        if (!(lp instanceof HsGridLayoutManager.LayoutParams)) {
...
//        }
    ...
    /**
     * Returns the current {@link SpanSizeLookup} used by the HsGridLayoutManager.
     *
     * @return The current {@link SpanSizeLookup} used by the HsGridLayoutManager.
     */
...
//            totalSpace = getWidth() - getPaddingRight() - getPaddingLeft();
            calculateLayoutItemBorders(getWidth() - getPaddingRight() - getPaddingLeft());
        } else {
            totalSpace = getWidth() - getPaddingRight() - getPaddingLeft();
        }
     ...
     * @param totalSpace Total available space after padding is removed
     */
    private void calculateLayoutItemBorders(int totalSpace) {
        mCachedLayoutBorders = calculateItemBorders(mCachedLayoutBorders, mSpanCount, totalSpace);
    }
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        WJLog.Companion.d("layoutChunk " + " layoutState.mCurrentPosition:" + layoutState.mCurrentPosition + " layoutState.mOffset:" + layoutState.mOffset + " layoutState.mAvailable:" + layoutState.mAvailable);
        ...
        while (count < mSpanCount && layoutState.hasMore(state) && remainingSpan > 0) {
            ...
            if (spanSize > mSpanCount) {
                throw new IllegalArgumentException("Item at position " + pos + " requires "
                        + spanSize + " spans but HsGridLayoutManager has only " + mSpanCount
                        + " spans.");
            }
            remainingSpan -= spanSize;
            if (remainingSpan < 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mOrientationHelper.getDecoratedMeasurement(view) != maxSize) {
                ...
                try {
                    declaredField = RecyclerView.LayoutParams.class.getDeclaredField("mDecorInsets");
                    declaredField.setAccessible(true);
                    final Rect decorInsets = (Rect) declaredField.get(lp);
        ...
        result.mConsumed = mCachedLayoutBorders[1];
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (DEBUG) {
                WJLog.Companion.d("laid out child at position " + getPosition(view)
                        + ", with l:" + (left + params.leftMargin)
                        + ", r:" + (right - params.rightMargin)
                        + ", t:" + (top + params.topMargin)
                        + ", b:" + (bottom - params.bottomMargin)
                        + ", span:" + params.mSpanIndex
                        + ", spanSize:" + params.mSpanSize);
            }
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec,
        ...
        if (alreadyMeasured) {
            try {
                Method shouldReMeasureChild = RecyclerView.LayoutManager.class.getDeclaredMethod("shouldReMeasureChild", View.class, int.class, int.class, RecyclerView.LayoutParams.class);
                shouldReMeasureChild.setAccessible(true);
                measure = (boolean) shouldReMeasureChild.invoke(this, child, widthSpec, heightSpec, lp);
            } catch (Exception e) {
                e.printStackTrace();
                measure = false;
            }
            ...
            try {
                Method shouldMeasureChild = RecyclerView.LayoutManager.class.getDeclaredMethod("shouldMeasureChild", View.class, int.class, int.class, RecyclerView.LayoutParams.class);
                shouldMeasureChild.setAccessible(true);
                measure = (boolean) shouldMeasureChild.invoke(this, child, widthSpec, heightSpec, lp);
            } catch (Exception e) {
                e.printStackTrace();
                measure = false;
            }
     ...
     *
     * @see HsGridLayoutManager#setSpanSizeLookup(SpanSizeLookup)
     */
    public abstract static class SpanSizeLookup {
        ...
        /**
         * Clears the span index cache. HsGridLayoutManager automatically calls this method when
        ...
        /**
         * Clears the span group index cache. HsGridLayoutManager automatically calls this method
         ...
         * {@link #getSpanSize(int)}. HsGridLayoutManager does not call this method for
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
        ...
        // HsLinearLayoutManager finds the last child. What we want is the child which has the same
        // spanIndex.
    ...
    /**
     * LayoutParams used by HsGridLayoutManager.
     * <p>
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
         ...
         * even if the layout is RTL. In a vertical HsGridLayoutManager, <b>leftmost</b> span is span
